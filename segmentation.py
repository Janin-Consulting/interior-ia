...
def apply_morphological_cleanup(
    ...
    cleaned_segmentation_float = cleaned_segmentation.astype(np.float32)
    # Utiliser un petit noyau pour préserver les détails importants
    median_filtered = cv2.medianBlur(cleaned_segmentation_float, 3)
...
def segment_image(
    ...
    try:
        ...
        # Récupération de la carte de segmentation
        seg_map = processor.post_process_semantic_segmentation(outputs, target_sizes=[image.size[::-1]])[0]
        
        # Nettoyage morphologique si activé
        if cfg["enable_morphological_cleanup"]:
            cleanup_start = time.time()
            seg_map_np = seg_map.cpu().numpy()
            seg_map_np = apply_morphological_cleanup(
                seg_map_np, 
                kernel_size=cfg["kernel_size"], 
                min_area_threshold=cfg["min_area_threshold"]
            )
            seg_map = torch.tensor(seg_map_np, device=device)
            cleanup_time = time.time() - cleanup_start
            logger.debug(f"Nettoyage morphologique terminé en {cleanup_time:.2f} secondes")
        
        # Colorisation de la carte de segmentation
        colorize_start = time.time()
        seg_map_np = seg_map.cpu().numpy()
        colored_seg = colorize_segmentation(seg_map_np)
        colorize_time = time.time() - colorize_start
        logger.debug(f"Colorisation de la segmentation terminée en {colorize_time:.2f} secondes")
        
        total_time = time.time() - start_time
        logger.info(f"Segmentation d'image terminée en {total_time:.2f} secondes")
        
        return seg_map, colored_seg
